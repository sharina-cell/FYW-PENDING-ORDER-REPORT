...
with tab_recon:
    st.markdown(
        ...
        "that were pushed but missed by the auto-match, then download the updated "
        "report — FILTERED DATA, PIVOT, and a **NOT PUSHED** sheet, all in one file."
    )
    ...
    # Fold in previously-confirmed manual entries so they stay applied across reruns/reuploads.
                    ...
                    if confirmed_rows.empty:
                        ...
                        for row in new_entries:
                            if row['eOrder Number'] not in existing_ids:
                                existing.append(row)
                                added += 1
                        st.success(f"Updated report — {added} order(s) moved out of Not Pushed and into FILTERED DATA/PIVOT.")
                        st.rerun()
    ...
    # ── Updated Report (FILTERED DATA + PIVOT + NOT PUSHED) ─────────────────────
    st.subheader("📄 Updated Report Preview")

    updated_df = merge_manual_orders(df_report, st.session_state.manual_confirmed)
    manual_row_count = len(updated_df) - len(df_report)

    p1, p2, p3 = st.columns(3)
    p1.metric("Filtered Data — Total Items", len(updated_df))
    p2.metric("Manually Added Rows", manual_row_count)
    p3.metric("Still Not Pushed to TC", total_missing)

    st.caption(
        "This preview reflects the base Pending Order Report plus any orders you've "
        "confirmed above. Download below to get FILTERED DATA, PIVOT, SUMMARY, "
        "per-brand sheets, and a NOT PUSHED sheet all in one workbook."
    )

    st.subheader("⬇️ Download Updated Report")
    with st.spinner("Generating updated report..."):
        updated_buffer = generate_excel(
            updated_df,
            report_date=datetime.combine(report_date, datetime.min.time()),
            not_pushed_results=results,
        )

    updated_filename = f"FYW_Pending_Orders_Updated_{report_date.strftime('%d%b%Y')}.xlsx"
    st.download_button(
