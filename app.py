...
with tab_recon:
        ...
        for label, res in results.items():
            ...
            with st.expander(f"🔴 {label} — {res['missing_count']} orders missing from TC"):
                ...
                st.markdown(
                    ...
                    "fill in what you know for each order below, then apply."
                )
                ...
                if st.button(f"✅ Apply confirmed rows — {label}", key=f"apply_{label}"):
                    ...
                    if confirmed_rows.empty:
                        ...
                        for row in new_entries:
                            if row['eOrder Number'] not in existing_ids:
                                existing.append(row)
                                added += 1
                        st.success(f"Applied {added} order(s) — moved out of Missing and into FILTERED DATA/PIVOT.")
                        st.rerun()
    ...
    with st.spinner("Generating reconciliation Excel..."):
        recon_buffer = generate_reconciliation_excel(
            results,
            report_date=datetime.combine(report_date, datetime.min.time()),
            manual_df=manual_confirmed_to_df(st.session_state.manual_confirmed),
        )
