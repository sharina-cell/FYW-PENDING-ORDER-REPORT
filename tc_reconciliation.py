...
MP_EXTRACTION_RULES = {
    'shopee': {'status_col': 'C',  'date_col': 'K', 'sla_col': 'I', 'sla_mode': 'direct'},
    'tiktok': {'status_col': 'B',  'date_col': 'X', 'sla_mode': 'plus1'},
    'lazada': {'status_col': 'BO', 'date_col': 'I', 'sla_mode': 'plus1'},
}
...
def extract_order_detail(label: str, file_obj, order_id: str, id_col_name: str = None) -> dict:
    ...
    if rules['sla_mode'] == 'direct':
        mp_sla = pd.to_datetime(col_val(rules['sla_col']), errors='coerce', dayfirst=True)
    else:
        mp_sla = ordered_date + pd.Timedelta(days=1) if pd.notna(ordered_date) else pd.NaT

    return {
        'eOrder Number':     str(order_id),
        'Order Item Status': status_str,
        'Payment Status':    payment_status,
        'Ordered Date':      ordered_date.strftime('%d/%m/%Y') if pd.notna(ordered_date) else '',
